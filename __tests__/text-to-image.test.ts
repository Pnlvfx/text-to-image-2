...
describe('the text-to-image-2 generator', () => {
  ...
  it('should return a promise', (done) => {
    expect(textToImage.generate('Hello world')).toBeInstanceOf(Promise);
    done();
  });

  it('should have a sync version', () => {
    expect(typeof textToImage.generateSync('Hello world')).toEqual('string');
  });

  it('should support debug in sync mode', () => {
    textToImage.generateSync('Hello world', {
      debug: true,
  ...
  it('should support custom filepaths in async debug mode', async () => {
    const baseDir = path.join(process.cwd(), 'test', 'custom_path');
    const filePath = path.join(baseDir, 'to', '1_path_debug.png');
    await textToImage.generate('Hello world', {
      debug: true,
      debugFilename: filePath,
    });
  ...
  it('should generate an image data url', async () => {
    const dataUri = await textToImage.generate('Hello world');
  ...
  it("should generate equal width but longer png when there's plenty of text", async () => {
    ...
    const uri2 = await textToImage.generate(
  ...
  it(String.raw`should create a new lines when a \n occurrs`, async () => {
    const uri1 = await textToImage.generate('Hello world');
  ...
  it(String.raw`should create a new lines when a multiple \n occurrs`, async () => {
    const uri1 = await textToImage.generate('Hello world\nhello again');
    const uri2 = await textToImage.generate('Hello world\n\n\nhello again');
  ...
  it('should use the font weight specified', async () => {
    const uri1 = await textToImage.generate('Lorem ipsum dolor sit amet.', {
      fontWeight: 'bold',
    });
    const uri2 = await textToImage.generate('Lorem ipsum dolor sit amet.', {
      fontWeight: 'normal',
    });
    ...
    const colorA = normalMap['#000000'];
    const colorB = normalMap['#ffffff'];

    if (!colorA || !colorB) throw new Error('Something went wrong!');

    // check that we have more black and less white in the bold text image
    expect(boldMap['#000000']).toBeGreaterThan(colorA);
    expect(boldMap['#ffffff']).toBeLessThan(colorB);
  ...
  it('should support right aligning text', async () => {
    ...
    const whitePixels = countWhitePixels(rightAlignData, 0, 0, 150, rightAlignData.height);
    expect(whitePixels).toBe(rightAlignData.height * 150);

    // expect some pixels on right side (from 150px) include non-white
    const nonWhitePixels = countWhitePixels(rightAlignData, 150, 0, rightAlignData.width, rightAlignData.height);
  ...
  it('should support left aligning text', async () => {
    ...
    const whitePixels = countWhitePixels(leftAlignData, 250, 0, leftAlignData.width, leftAlignData.height);
    expect(whitePixels).toBe(leftAlignData.height * 150);

    // expect some pixels on left side (up to 250px) to be non-white
    const nonWhitePixels = countWhitePixels(leftAlignData, 0, 0, 250, leftAlignData.height);
  ...
  it('should support center aligning text', async () => {
    ...
    const leftWhitePixels = countWhitePixels(centerAlignData, 0, 0, 80, centerAlignData.height);
    expect(leftWhitePixels).toBe(centerAlignData.height * 80);

    // expect all pixels on left side (last 80px) to be white
    const rightWhitePixels = countWhitePixels(centerAlignData, centerAlignData.width - 80, 0, centerAlignData.width, centerAlignData.height);
    ...
    const centerWhitePixels = countWhitePixels(centerAlignData, 80, 0, centerAlignData.width - 80, centerAlignData.height);
    expect(centerWhitePixels).toBeLessThan(centerAlignData.height * (centerAlignData.width - 160));
  ...
  it('should warn if the text is longer than customHeight', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Text is longer than customHeight, clipping will occur.');
  });
  ...
  it('should support vertical align', async () => {
    ...
    const topWhitePixels = countWhitePixels(verticalCenter, 0, 0, verticalCenter.width, 35);
    expect(topWhitePixels).toBe(verticalCenter.width * 35);

    // middle pixel rows should contain non-whites
    const centerWhitePixels = countWhitePixels(verticalCenter, 0, 35, verticalCenter.width, verticalCenter.height - 35);
    expect(centerWhitePixels).toBeLessThan(verticalCenter.width * (verticalCenter.height - 70));

    // bottom 35 rows should be white
    const bottomWhitePixels = countWhitePixels(verticalCenter, 0, verticalCenter.height - 35, verticalCenter.width, verticalCenter.height);
  ...
  it('should support custom font paths', async () => {
    const uri = await textToImage.generate('S', {
  ...
  it('should support very long inputs', async () => {
    const uri = await textToImage.generate(longInput, {});
  ...
  it('should not duplicate text with transparent background', async () => {
    ...
    const { frames } = await readImageData(uriToBuf(uri));
    const data = frames.at(0)?.data;
    if (!data) throw new Error('Error');
    // the top 100 pixel rows should not have any data
    const topRowsData = data.subarray(0, 400 * 100 * 4); // 400px wide, 100px high, 4 values per pixel
  ...
  it('should support speech bubble tail', async () => {
    ...
    const uri = await textToImage.generate('This is Speech bubble', {
      maxWidth: width,
    ...
    const whitePixels = countWhitePixels(imageData, center - 0.5, height, center + 0.5, height + bubbleTailSize.height);
